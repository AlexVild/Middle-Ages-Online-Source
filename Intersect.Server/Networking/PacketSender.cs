...
namespace Intersect.Server.Networking
{
    ...
    public static partial class PacketSender
    {
        ...
        /// <param name="type">The type of message we are sending.</param>
        /// <param name="mapId">The Map we are sending this message to (and its surroundings).</param>
        /// <param name="mapInstanceId">The ID of the instance to send the message to.</param>
        /// <param name="color">The color assigned to this message.</param>
        /// <param name="target">The sender of this message, should we decide to respond from the client.</param>
        /// <returns></returns>
        ...
        )
        {
            ...
            if (MapController.TryGetInstanceFromMap(mapId, mapInstanceId, out var mapInstance))
            {
                ...
                {
                    mapInstance.AddBatchedAnimation(new PlayAnimationPacket(animId, targetType, entityId, mapId, x, y, direction, projectileHitAnim));
                }
                else
                {
                    SendDataToProximityOnMapInstance(mapId, mapInstanceId, new PlayAnimationPacket(animId, targetType, entityId, mapId, x, y, direction, projectileHitAnim), null, TransmissionMode.Any);
                }
            }
        }
        ...
        public static void SendGameObjects(Client client, GameObjectType type, List<GameObjectPacket> packetList = null)
        {
            switch (type)
            {
                ...
                case GameObjectType.LootTable:
                    foreach (var obj in LootTableDescriptor.Lookup)
                    {
                        SendGameObject(client, obj.Value, false, false, packetList);
                    }
                    break;
                ...
                case GameObjectType.Recipe:
                    foreach (var obj in RecipeDescriptor.Lookup)
                    {
                        SendGameObject(client, obj.Value, false, false, packetList);
                    }
                    break;
                ...
                case GameObjectType.Dungeon:
                    foreach (var obj in DungeonDescriptor.Lookup)
                    {
                        SendGameObject(client, obj.Value, false, false, packetList);
                    }
                    break;
                ...
                case GameObjectType.Enhancement:
                    foreach (var obj in EnhancementDescriptor.Lookup)
                    {
                        SendGameObject(client, obj.Value, false, false, packetList);
                    }
                    break;
        ...
        public static void SendResourceLockPacket(Player player, bool resourceLock, double harvestBonus, long harvestsRemaining, Guid resourceId)
        {
            player?.SendPacket(new ResourceLockPacket(resourceLock, harvestBonus, (int)harvestsRemaining, resourceId));
        }
        ...
        public static void SendTimerPacket(Player player, TimerInstance timer)
        {
            ...
            var elapsedTimeOnRep = timer.ElapsedTime;

            player?.SendPacket(new TimerPacket(timer.DescriptorId, timer.TimeRemaining, elapsedTimeOnRep, descriptor.Type, descriptor.DisplayName, descriptor.ContinueAfterExpiration));
        }
    ...
    public static partial class PacketSender
    {
        ...
        public static void SendRecordPageTo(Player player, List<RecordDto> records, int currentPage, string highlightedPlayer)
        {
            if (player == null)
            {
                return;
        ...
        public static void SendOpenLootPacketTo(Player player, string title, LootAnimType animationType, bool disableBanking = false)
        {
            if (player == null)
            {
                return;
            ...
            player?.SendPacket(new OpenLootRollPacket(lootObjs, title, animationType, disableBanking));
        }
        ...
        public static void SendOpenDeconstructor(Player player, float multiplier)
        {
